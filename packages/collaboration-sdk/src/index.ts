import * as Automerge from './automerge-lite.js';
...
export interface CollaborationSessionServiceOptions {
  seedSessions?: Array<{ participants: string[]; poses?: PoseSnapshot[] }>;
  initialState?: SessionDetail[];
  onChange?: (sessions: SessionDetail[]) => void;
}

export class CollaborationSessionService {
  private readonly sessions: Map<string, SessionDetail> = new Map();
  private readonly onChange?: (sessions: SessionDetail[]) => void;

  constructor(options: CollaborationSessionServiceOptions = {}) {
    const { seedSessions, initialState, onChange } = options;
  ...
  private hydrate(sessions: SessionDetail[]): void {
    for (const session of sessions) {
      const poses = [...session.poses].sort((a, b) => b.createdAt - a.createdAt);
      const normalised: SessionDetail = {
        ...session,
        poses,
        poseCount: poses.length,
        lastUpdated: poses[0]?.createdAt ?? session.lastUpdated ?? Date.now()
      };
      this.sessions.set(normalised.id, normalised);
    }
  }

  private snapshotSessions(): SessionDetail[] {
    return Array.from(this.sessions.values()).map(session => ({
      ...session,
      poses: session.poses.map(pose => ({ ...pose }))
    }));
  }
  ...
  private notifyChange(): void {
    ...
    this.onChange(this.snapshotSessions());
  }
  ...
  recordPose(
    ...
    const result = this.writeSession(sessionId, draft => {
...
export { FileSessionStore } from './persistence.js';
